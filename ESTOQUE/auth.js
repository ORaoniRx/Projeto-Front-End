const auth = {
    ...
    check() {
        ...
        if (user) {
            section.innerHTML = `<span class="text-white me-2 small"<Olá, <b> ${user} </b> 
        </span><button class="btn btn-sm btn-danger" 
        onclick="auth.logout()">Sair</button>`;
        } else {
            section.innerHTML = `<button class="btn btn-sm  btn-outline-light onclikck="auth.toggleLoginForm()">Login</button>`
        }
    },
